...
//            //Left
//            List<Entity> leftSlotHoles = GetSlotHoles(GetSumPoint(rafter[3].MidPoint, lengthmargin_F1 + lengthGap_G1 / 2, 0), slotholeHT, widthGap_E, slotholeWD, lengthGap_G1,scaleValue);
////            List<Entity> leftHoles = GetHoles(GetSumPoint(rafter[3].MidPoint, lengthmargin_F1 + lengthGap_G1 / 2, 0), slotholeHT, lengthGap_G1, widthGap_E,scaleValue);

//            //Right
//            List<Entity> rightSlotHoles = GetSlotHoles(GetSumPoint(rafter[1].MidPoint, -(lengthmargin_F1 + lengthGap_G1 / 2), 0), slotholeHT, widthGap_E, slotholeWD, lengthGap_G1,scaleValue);
//            //List<Entity> rightHoles = GetHoles(GetSumPoint(rafter[1].MidPoint, -(lengthmargin_F1 + lengthGap_G1 / 2), 0), slotholeHT, lengthGap_G1, widthGap_E, scaleValue);
...
//        }
                                    ...
                                    NColumnRafterModel rafterModel = null, double leftHoleQty = 0, double rightHoleQty = 0,double padThk=0, bool rightCenterPosition = false,  double scaleValue=1)
        {
            ...
            double F = valueService.GetDoubleValue(rafterModel.F);    //SHELL ID/4 : NOT USED
            double G = valueService.GetDoubleValue(rafterModel.G);    //SHELL ID/4 : NOT USED
            ...
            // Left Hole
            slotHoleList.AddRange(GetHoles(GetSumPoint(leftHoleCenterPoint,0,0),leftHoleQty, holeDia,scaleValue, leftHoleDistanceX,leftHoleDistanceY));
            // Right Hole
            slotHoleList.AddRange(GetHoles(GetSumPoint(rightHoleCenterPoint, 0, 0), rightHoleQty, holeDia, scaleValue, rightHoleDistanceX, rightHoleDistanceY));
        ...
        public DrawEntityModel RafterSideClipDetail(Point3D refPoint,Point3D roofPoint, object selModel, double scaleValue, NRafterSupportClipShellSideModel rafterSupportClipModel,double shellThickness,double drawType = 1, bool topViewLeft=true)
        {
            DrawEntityModel drawList = new DrawEntityModel();
            List<Entity> outlinesList = new List<Entity>();
            List<Entity> holeList = new List<Entity>();
            List<Entity> topViewList = new List<Entity>();
            List<Entity> topViewCenterLineList = new List<Entity>();
            Point3D workingPoint = GetSumPoint(refPoint, 0, 0);

            double A = valueService.GetDoubleValue(rafterSupportClipModel.A);  //rafterWidth
            double A1 = valueService.GetDoubleValue(rafterSupportClipModel.A1);   //roof to clip Distance 
            double B = valueService.GetDoubleValue(rafterSupportClipModel.B);  //clip To rafter hole center : NOT USED
            double B1 = valueService.GetDoubleValue(rafterSupportClipModel.B1);  //working Point to pad Start Point
            double C = valueService.GetDoubleValue(rafterSupportClipModel.C); //clipRightWidth
            double C1 = valueService.GetDoubleValue(rafterSupportClipModel.C1);  //pad Start Point to clip start Point
            double D = valueService.GetDoubleValue(rafterSupportClipModel.D);  //rafter to hole : NOT USED
            double D1 = valueService.GetDoubleValue(rafterSupportClipModel.D1);  //workingPoint부터 rafter까지의 x축 거리
            double E = valueService.GetDoubleValue(rafterSupportClipModel.E);  //hole to hole width gap
            double E1 = valueService.GetDoubleValue(rafterSupportClipModel.E1);  //slothole to clip right line
            double F = valueService.GetDoubleValue(rafterSupportClipModel.F); //Pad Width\
            double F1 = valueService.GetDoubleValue(rafterSupportClipModel.F1); //rafter to hole
            double G = valueService.GetDoubleValue(rafterSupportClipModel.G);  //clip left width
            double G1 = valueService.GetDoubleValue(rafterSupportClipModel.G1);  //hole to hole length gap
            double H1 = valueService.GetDoubleValue(rafterSupportClipModel.H1);  //clipBottomLength

            double K1 = valueService.GetDoubleValue(rafterSupportClipModel.K1);  //clip Thickness
            double I1 = valueService.GetDoubleValue(rafterSupportClipModel.I1);  //clip Pad Thickness

            double holeQty = valueService.GetDoubleValue(rafterSupportClipModel.HoleQty);
            double slotholeHT = valueService.GetDoubleValue(rafterSupportClipModel.SlotHoleHeight);
            double slotholeWD = valueService.GetDoubleValue(rafterSupportClipModel.SlotHoleLength);

            double t = shellThickness;   //현재 있는 값은 t2, t3등도 전부 동일함
            ...
            //rafter And Holes
            List<Entity> slotHoles = GetSlotHoles(GetSumPoint(rafter[3].MidPoint, F1 + G1 / 2, 0), slotholeHT, slotholeWD, G1, E,scaleValue);
            List<Entity> holes = GetHoles(GetSumPoint(rafter[3].MidPoint, F1 + G1 / 2, 0), holeQty, slotholeHT, G1, E,scaleValue);
            ...
            //////////////////////TOPVIEW/////////////////////
            double padHeight_t = I1;
            double clipThk_k1 = K1;
            ...
            if (!topViewLeft)
                clipTop=GetRectangle(GetSumPoint(topViewWorkingPoint, t, -clipThk_k1), clipThk_k1, G, RECTANGLUNVIEW.NONE);
            ...
            topViewCenterLineList.AddRange(editingService.GetCenterLine(GetSumPoint(topViewWorkingPoint, t + padTOhole1, clipThk_k1),
                                                                        GetSumPoint(topViewWorkingPoint, t + padTOhole1, -clipThk_k1), exLength, scaleValue));
            topViewCenterLineList.AddRange(editingService.GetCenterLine(GetSumPoint(topViewWorkingPoint, t + padTOhole2, clipThk_k1),
                                                                        GetSumPoint(topViewWorkingPoint, t + padTOhole2, -clipThk_k1), exLength, scaleValue));
            topViewCenterLineList.AddRange(editingService.GetCenterLine(GetSumPoint(topViewWorkingPoint, t + padTOhole3, clipThk_k1),
                                                                        GetSumPoint(topViewWorkingPoint, t + padTOhole3, -clipThk_k1), exLength, scaleValue));
            topViewCenterLineList.AddRange(editingService.GetCenterLine(GetSumPoint(topViewWorkingPoint, t + padTOhole4, clipThk_k1),
                                                                        GetSumPoint(topViewWorkingPoint, t + padTOhole4, -clipThk_k1), exLength, scaleValue));
            ...
            else if (drawType == 2)
            {
                ...
                outlinesList.AddRange(new Entity[] {
                    //columnOuterLine, columnInnerLine, 
                    });

                styleService.SetLayerListEntity(ref topViewCenterLineList, layerService.LayerCenterLine);
                drawList.outlineList.AddRange(topViewCenterLineList);
            }
            else if (drawType == 3)
            {
                //Front View
            ...
            drawList.outlineList.AddRange(holeList);
        ...
        private List<Entity> GetHoles(Point3D refPoint,double holeQty,  double holeDiameter, double scaleValue , double xGap=0, double yGap = 0)
        {
        ...
        private List<Entity> GetSlotHoles(Point3D refPoint, double holeRadius, double slotHoleLength, double lengthGap, double widthGap = 0,double scaleValue=1)
        {
            //refPoint는 홀과 홀 사이의 중앙인 점을 입력해주시면됩니다
            //lengthGap=0인경우는 2개, lengthGap에 값이 입력된 경우는 홀이 4개로 그려집니다 :)
            List<Entity> newList = new List<Entity>();
            ...
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    ...
                    centerLineList.AddRange(editingService.GetCenterLine(GetSumPoint(circleCenterPoint, -slotHoleLength / 2, 0),
                                                     GetSumPoint(circleCenterPoint, slotHoleLength / 2, 0), exCenterLength, scaleValue));
                    centerLineList.AddRange(editingService.GetCenterLine(GetSumPoint(circleCenterPoint, 0, -holeRadius / 2),
                                                                         GetSumPoint(circleCenterPoint, 0, holeRadius / 2), exCenterLength, scaleValue));
        ...
        public DrawEntityModel ColumnSideTopPlate(Point3D refPoint,double girderHeight,double girderDegree,double drawType, double scaleValue, NColumnGirderModel girderModel, NColumnSideTopSupportModel sideSupportModel)
        {
            DrawEntityModel drawList = new DrawEntityModel();
            Point3D referencePoint = GetSumPoint(refPoint, 0, 0);
            List<Entity> newList = new List<Entity>();
            List<Entity> circleList = new List<Entity>();
            List<Entity> slotList=new List<Entity>();

            double A = valueService.GetDoubleValue(girderModel.A);   //rafter Width
            double B = valueService.GetDoubleValue(girderModel.B);    //width Gap
            double C = valueService.GetDoubleValue(girderModel.C);    //rafter width margin
            double H1 = valueService.GetDoubleValue(girderModel.H1);    //rafter length gap
            double I1 = valueService.GetDoubleValue(girderModel.I1);    //rafter width margin
            double J1 = valueService.GetDoubleValue(girderModel.J1);    //rafter to plate

            double D = valueService.GetDoubleValue(sideSupportModel.D);    //center to rafter
            double E = valueService.GetDoubleValue(sideSupportModel.E);    //center to hole
            double A1 = valueService.GetDoubleValue(sideSupportModel.A1);   //thk of plate
            double B1 = valueService.GetDoubleValue(sideSupportModel.B1);    //wing short 00width
            double C1 = valueService.GetDoubleValue(sideSupportModel.C1);    //wing Length
            double D1 = valueService.GetDoubleValue(sideSupportModel.D1);    //center to rafter
            double E1 = valueService.GetDoubleValue(sideSupportModel.E1);    //wing long width
            double F1 = valueService.GetDoubleValue(sideSupportModel.F1);    //center to hole
            ...
            //TEMP VALUE
            PipeModel pipeModel = GetPipeModel(sideSupportModel.ColumnSize);
            double pipeOD = valueService.GetDoubleValue(pipeModel.OD);
            double pipeRadius = pipeOD / 2;
            double circleRad = pipeRadius+ C1 + D1;
            double rafterRad = Utility.DegToRad(girderDegree);  // Girder Degree
            double settingRad = Utility.DegToRad(90);
            double girder = girderHeight; // Girder Height

            Point3D workingPoint = GetSumPoint(refPoint, 0, 0);

            //rafter And Holes
            Point3D rafterPointA = GetSumPoint(workingPoint, -circleRad, -A / 2); //좌측하단
            Point3D rafterPointB = GetSumPoint(workingPoint, -D, -A / 2); //우측하단
            Point3D rafterPointC = GetSumPoint(workingPoint, -circleRad, A / 2); //좌측상단
            Point3D rafterPointD = GetSumPoint(workingPoint, -D, A / 2); //우측상단

            //First
            Line rafterUnder1 = new Line(GetSumPoint(rafterPointA, 0, 0), GetSumPoint(rafterPointB, 0, 0));
            Line rafterRight1 = new Line(GetSumPoint(rafterPointD, 0, 0), GetSumPoint(rafterPointB, 0, 0));
            Line rafterTop1 = new Line(GetSumPoint(rafterPointD, 0, 0), GetSumPoint(rafterPointC, 0, 0));

            List<Entity> holes1 = GetHoles(GetSumPoint(rafterRight1.MidPoint, -I1 - H1 / 2, 0), slotholeHT, H1, B,scaleValue);

            List<Entity> rafterNslot1 = new List<Entity>();
            rafterNslot1.Add(rafterUnder1);
            rafterNslot1.Add(rafterRight1);
            rafterNslot1.Add(rafterTop1);
            rafterNslot1.AddRange(holes1);

            foreach (Entity eachEntity in rafterNslot1)
            {
                //회전기준점 : workingpoint
                eachEntity.Rotate(settingRad - rafterRad / 2, Vector3D.AxisMinusZ, GetSumPoint(workingPoint, 0, 0));
            }

            //Second
            Line rafterUnder2 = new Line(GetSumPoint(rafterPointA, 0, 0), GetSumPoint(rafterPointB, 0, 0));
            Line rafterRight2 = new Line(GetSumPoint(rafterPointD, 0, 0), GetSumPoint(rafterPointB, 0, 0));
            Line rafterTop2 = new Line(GetSumPoint(rafterPointD, 0, 0), GetSumPoint(rafterPointC, 0, 0));

            List<Entity> holes2 = GetHoles(GetSumPoint(rafterRight2.MidPoint, -I1 - H1 / 2, 0), slotholeHT, H1, B, scaleValue);

            List<Entity> rafterNslot2 = new List<Entity>();
            rafterNslot2.Add(rafterUnder2);
            rafterNslot2.Add(rafterRight2);
            rafterNslot2.Add(rafterTop2);
            rafterNslot2.AddRange(holes2);

            foreach (Entity eachEntity in rafterNslot2)
            {
                //회전기준점 : workingpoint
                eachEntity.Rotate(settingRad + rafterRad / 2, Vector3D.AxisMinusZ, GetSumPoint(workingPoint, 0, 0));
            }

            if (drawType == 0 ) //COLUMN SIDE TOP DETAIL
            {
                List<Line> topPlate = GetRectangle(GetSumPoint(workingPoint, -circleRad / 2, girder), A1, circleRad, RECTANGLUNVIEW.NONE);
                List<Line> leftGirder = GetDoubleLineRectangle(GetSumPoint(workingPoint, -(circleRad + pipeOD) / 2, 0), girder, circleRad / 2, A1, RECTANGLUNVIEW.LEFT);
                List<Line> rightGirder = GetDoubleLineRectangle(GetSumPoint(workingPoint, pipeOD / 2, 0), girder, circleRad / 2, A1, RECTANGLUNVIEW.RIGHT);
                List<Line> bottomPlate = GetRectangle(GetSumPoint(workingPoint, -circleRad / 2, -A1), A1, circleRad, RECTANGLUNVIEW.NONE);
                newList.AddRange(topPlate);
                newList.AddRange(leftGirder);
                newList.AddRange(rightGirder);
                newList.AddRange(bottomPlate);


                //hole Line
                Line holeLine1 = new Line(GetSumPoint(workingPoint, -(E + I1), -A1 * 2), GetSumPoint(workingPoint, -(E + I1), girder + A1 * 2));
                Line holeLine2 = new Line(GetSumPoint(workingPoint, -E, -A1 * 2), GetSumPoint(workingPoint, -E, girder + A1 * 2));
                Line holeLine3 = new Line(GetSumPoint(workingPoint, E, -A1 * 2), GetSumPoint(workingPoint, E, girder + A1 * 2));
                Line holeLine4 = new Line(GetSumPoint(workingPoint, E + I1, -A1 * 2), GetSumPoint(workingPoint, E + I1, girder + A1 * 2));

                //왼쪽날개
                Line shortLeftWingWidthLine = new Line(GetSumPoint(workingPoint, -pipeOD / 2 - C1, -A1), GetSumPoint(workingPoint, -pipeOD / 2 - C1, -A1 - E1));
                Line longLeftWingWidthLine = new Line(GetSumPoint(workingPoint, -pipeOD / 2, -A1), GetSumPoint(workingPoint, -pipeOD / 2, -A1 - B1));
                Line LeftSlideLine = new Line(GetSumPoint(shortLeftWingWidthLine.EndPoint, 0, 0), GetSumPoint(longLeftWingWidthLine.EndPoint, -E1, 0));
                Line LeftTopLine = new Line(GetSumPoint(longLeftWingWidthLine.EndPoint, 0, 0), GetSumPoint(longLeftWingWidthLine.EndPoint, -E1, 0));
                //오른쪽날개
                Line shortRightWingWidthLine = new Line(GetSumPoint(workingPoint, pipeOD / 2 + C1, -A1), GetSumPoint(workingPoint, pipeOD / 2 + C1, -A1 - E1));
                Line longRightWingWidthLine = new Line(GetSumPoint(workingPoint, pipeOD / 2, -A1), GetSumPoint(workingPoint, pipeOD / 2, -A1 - B1));
                Line RightSlideLine = new Line(GetSumPoint(shortRightWingWidthLine.EndPoint, 0, 0), GetSumPoint(longRightWingWidthLine.EndPoint, E1, 0));
                Line RightTopLine = new Line(GetSumPoint(longRightWingWidthLine.EndPoint, 0, 0), GetSumPoint(longRightWingWidthLine.EndPoint, E1, 0));

                newList.AddRange(new Entity[] {
                    holeLine1, holeLine2, holeLine3, holeLine4,
                    shortLeftWingWidthLine, longLeftWingWidthLine, LeftSlideLine, LeftTopLine,
                    shortRightWingWidthLine, longRightWingWidthLine, RightSlideLine, RightTopLine,

                });
            }
            
            if (drawType == 1 || drawType == 3) //Circle Type
            {
                slotList.AddRange(holes1);
                slotList.AddRange(holes2);

                Circle supportCircle = new Circle(workingPoint, circleRad);
                styleService.SetLayer(ref supportCircle, layerService.LayerOutLine);
                circleList.Add(supportCircle);

                Circle pipeCircle = new Circle(workingPoint, pipeRadius);
                styleService.SetLayer(ref pipeCircle, layerService.LayerHiddenLine);
                circleList.Add(pipeCircle);
            }
            
            if (drawType == 2 || drawType == 3) //Hex Type
            {
                slotList.AddRange(holes1);
                slotList.AddRange(holes2);

                Line hexUnderLine = new Line(GetSumPoint(rafterRight1.EndPoint, 0, 0), GetSumPoint(rafterRight2.StartPoint, 0, 0));
                Line hexLUnderLine = new Line(GetSumPoint(rafterRight1.EndPoint, 0, 0), GetSumPoint(rafterRight1.EndPoint, -J1, 0));
                hexLUnderLine.Rotate(settingRad - rafterRad / 2, Vector3D.AxisMinusZ, hexUnderLine.StartPoint);
                Line hexLSlideLine = new Line(GetSumPoint(hexLUnderLine.EndPoint, 0, 0), GetSumPoint(hexLUnderLine.EndPoint, -A, 0));
                hexLSlideLine.Rotate(settingRad * 2 - rafterRad / 2, Vector3D.AxisMinusZ, hexLSlideLine.StartPoint);

                Line hexRUnderLine = new Line(GetSumPoint(rafterRight2.StartPoint, 0, 0), GetSumPoint(rafterRight2.StartPoint, -J1, 0));
                hexRUnderLine.Rotate(settingRad + rafterRad / 2, Vector3D.AxisMinusZ, hexUnderLine.EndPoint);
                Line hexRSlideLine = new Line(GetSumPoint(hexRUnderLine.EndPoint, 0, 0), GetSumPoint(hexRUnderLine.EndPoint, -A, 0));
                hexRSlideLine.Rotate(rafterRad / 2, Vector3D.AxisMinusZ, hexRSlideLine.StartPoint);

                Line hexTopLine = new Line(GetSumPoint(hexLSlideLine.EndPoint, 0, 0), GetSumPoint(hexRSlideLine.EndPoint, 0, 0));
                ...
                newList.AddRange(new Entity[] {
                    hexUnderLine,
                    hexLUnderLine, hexLSlideLine,
                    hexRUnderLine, hexRSlideLine,
                    hexTopLine
                });
            }
            ...
            newList.AddRange(circleList);
            newList.AddRange(slotList);
            drawList.outlineList.AddRange(editingService.GetRotate(newList, referencePoint, Utility.DegToRad(180)));
        ...
        public List<Entity> GetColumnRib(Point3D refPoint, double distanceWorkPoint, double scaleValue, NColumnBaseSupportModel padModel)
        {
            ...
            //outlinesList.AddRange(ribList);
            outlinesList.AddRange(guideBottomBox); // 0: bottomLine, 1: RightLine, 2: LeftLine

            outlinesList.AddRange(new Entity[] {
                ribDiagonalLine, ribChamferLine,
                ribArc,
                ribTopLine, ribTopRightLine,  
        ...
        public List<Entity> GetColumnPipe(Point3D refPoint, double outerDiameter, double height, VIEWPOSITION viewPosition, bool isInnerView = false, double thk = 0)
        {
            ...
            if (viewPosition == VIEWPOSITION.NONE)
            {
                outlinesList.Add(guideLineBottom);
            }
        ...
        public DrawEntityModel DrawColumnBaseSupport_Side_WideView(Point3D refPoint, object selModel, double scaleValue, double PipeHeight,double selBottomPlateThk,double selTopPlateThk, NColumnBaseSupportModel columnBaseSupport,AngleSizeModel angleModel,PipeModel selPipe, double selSlopeRadian = 0, bool isMirror = false)
        {
            DrawEntityModel drawList = new DrawEntityModel();
            Point3D referencePoint = GetSumPoint(refPoint, 0, 0);
            List<Entity> newList = new List<Entity>();
            ...
            double pipeOD = valueService.GetDoubleValue(selPipe.OD);// PipeOD
            double pipeThk = valueService.GetDoubleValue(selPipe.SCH40S_THK); // Pipe THK : SCH.40S

            double ribLength = valueService.GetDoubleValue(columnBaseSupport.A); // A
            double ribWidth = valueService.GetDoubleValue(columnBaseSupport.A1); // A1
            double sleeveLength = valueService.GetDoubleValue(columnBaseSupport.B);//pipeOD + 26; // B
            double baseTopPlateLength = valueService.GetDoubleValue(columnBaseSupport.C); // C
            double baseTopPlateThk = valueService.GetDoubleValue(columnBaseSupport.I1); // I1
            double ribTopFlat = valueService.GetDoubleValue(columnBaseSupport.D1); // D1
            
            double sleeveWidth = valueService.GetDoubleValue(columnBaseSupport.B1); //500; // B1 = a1-c1 // data shee 오류  200으로 나옴
            double RibTopWidth = valueService.GetDoubleValue(columnBaseSupport.C); // C1
            double radius = valueService.GetDoubleValue(columnBaseSupport.F1); // F1
            double sleeveThk = valueService.GetDoubleValue(columnBaseSupport.H1); // H1
            double baseBottomPlateLength = valueService.GetDoubleValue(columnBaseSupport.E); // E
            double baseBottomPlateThk = selBottomPlateThk; // D BTM plate thk.  계산서에서 입력된 값
            double gapEachAngle = valueService.GetDoubleValue(columnBaseSupport.F); // F
            double angleHeight = valueService.GetDoubleValue(angleModel.A); // J1  - string 일부 읽기
            double angleThk = valueService.GetDoubleValue(angleModel.t); // J1
            double angleLength = valueService.GetDoubleValue(columnBaseSupport.K1); // K1
            double distanceSleeve = valueService.GetDoubleValue(columnBaseSupport.E1); // E1
            ...
            double slopeRadian = selSlopeRadian;//Utility.DegToRad(SlopeDeg);

            ////////////////////////////////
            /// Draw 
            ////////////////////////////////
            double basePlateHeight = baseTopPlateThk + baseBottomPlateThk;
            double pipeHeightAdj = PipeHeight - basePlateHeight - selTopPlateThk;
            // Pipe
            entityList.AddRange(GetColumnPipe(GetSumPoint(referencePoint, 0, basePlateHeight), pipeOD, pipeHeightAdj, VIEWPOSITION.NONE));


            // Pipe BottoisBottomSlopem Slope : default / 모양


            Line pipeBottomLine = (Line)entityList[0];
            Point3D pipeBottomCenterPoint = pipeBottomLine.MidPoint;
            Line guideLeftSlopeLine = new Line(pipeBottomCenterPoint, GetSumPoint(pipeBottomCenterPoint, -PipeHeight, 0));

            //Line guideRightSlopeLine = new Line(pipeBottomCenterPoint, GetSumPoint(pipeBottomCenterPoint, PipeHeight, basePlateHeight));
            ...
            //List<Entity> PipeList = new List<Entity>();
            //List<Entity> sleeveList = new List<Entity>();
            //List<Entity> ribLeftList = new List<Entity>();
            //List<Entity> baseTopPlate = new List<Entity>();
            //List<Entity> BaseBottomPlateList = new List<Entity>();




            // Draw : Pipe Bottom Slope
            double dFlip = 1;
            double LeftFlip = 1;
            ...
            // Set : Pipe Bottom Center Point
            Point3D pipeBottomCP = GetSumPoint(referencePoint, 0, basePlateHeight);

            Line pipeLeftLine = (Line)entityList[0];
            Line pipeRightLine = (Line)entityList[1];

            Point3D pipeBottomLeftPoint = CopyPoint(pipeLeftLine.EndPoint);
            Point3D pipeBottomRightPoint = CopyPoint(pipeRightLine.EndPoint);

            pipeLeftLine.EndPoint = GetSumPoint(pipeBottomLeftPoint, 0, -PipeHeight);
            pipeRightLine.EndPoint = GetSumPoint(pipeBottomRightPoint, 0, -PipeHeight);

            // Extend Pipe Line : 
            Line pipeBottomLeftLine = new Line(CopyPoint(pipeBottomCP), GetSumPoint(pipeBottomLeftPoint, -PipeHeight, 0));
            Line pipeBottomRightLine = new Line(CopyPoint(pipeBottomCP), GetSumPoint(pipeBottomRightPoint, PipeHeight, 0));

            pipeBottomLeftLine.Rotate(slopeRadian, Vector3D.AxisZ, pipeBottomCP);
            pipeBottomRightLine.Rotate(slopeRadian * dFlip, Vector3D.AxisZ, pipeBottomCP);

            Point3D[] intersectLeft = pipeLeftLine.IntersectWith(pipeBottomLeftLine);
            Point3D[] intersectRight = pipeRightLine.IntersectWith(pipeBottomRightLine);

            pipeLeftLine.EndPoint = pipeBottomLeftLine.EndPoint = CopyPoint(intersectLeft[0]);
            pipeRightLine.EndPoint = pipeBottomRightLine.EndPoint = CopyPoint(intersectRight[0]);

            outlinesList.AddRange(new Entity[] {
                pipeBottomLeftLine, pipeBottomRightLine
            });
            //
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (true)
                {
                    ...
                    {
                        guideLeftSlopeLine = new Line(pipeBottomCenterPoint, GetSumPoint(pipeBottomCenterPoint, -PipeHeight, 0));
                        dFlip = -1;
                    }
                    ...
                    {
                        guideLeftSlopeLine = new Line(pipeBottomCenterPoint, GetSumPoint(pipeBottomCenterPoint, -PipeHeight, 0));
                        guideLeftSlopeLine.Rotate(slopeRadian * dFlip, Vector3D.AxisZ, pipeBottomCenterPoint);
                    }

                    // Draw : Rib
                    List<Entity> ribLeftList = new List<Entity>();
                    ribLeftList = GetColumnRib(GetSumPoint(referencePoint, -(distanceRib + radiusOuter), basePlateHeight), distanceWorkPoint, scaleValue, columnBaseSupport);//, guideLeftSlopeLine);

                    Line ribBottomLn = (Line)ribLeftList[0];
                    Line ribRightLn = (Line)ribLeftList[1];
                    Line ribLeftLn = (Line)ribLeftList[2];
                    Line ribDiagonalLn = (Line)ribLeftList[3];
                    Line ribChamferLn = (Line)ribLeftList[4];

                    Line guideRibLeftLn = new Line(GetSumPoint(ribBottomLn.StartPoint, 0, ribWidth), GetSumPoint(ribBottomLn.StartPoint, 0, -ribWidth));
                    Line guideRibRightLn = new Line(GetSumPoint(ribRightLn.EndPoint, 0, ribWidth), GetSumPoint(ribRightLn.EndPoint, 0, -ribWidth));

                    Point3D crossLeftPoint = GetIntersectWidth(guideRibLeftLn, guideLeftSlopeLine);
                    Point3D crossRightPoint = GetIntersectWidth(guideRibRightLn, guideLeftSlopeLine);
                    Line guideChamferRibLn = new Line(crossRightPoint, guideLeftSlopeLine.EndPoint);
                    guideChamferRibLn.TrimAt(ribChamfer, false);


                    ribRightLn.EndPoint = GetSumPoint(crossRightPoint, 0, ribChamfer);

                    ribChamferLn.StartPoint = ribRightLn.EndPoint;
                    ribChamferLn.EndPoint = guideChamferRibLn.EndPoint;

                    ribBottomLn.StartPoint = crossLeftPoint;
                    ribBottomLn.EndPoint = ribChamferLn.EndPoint;

                    ribLeftLn.StartPoint = crossLeftPoint;
                    ribLeftLn.EndPoint = GetSumPoint(crossLeftPoint, 0, ribTopFlat);

                    ribDiagonalLn.EndPoint = CopyPoint(ribLeftLn.EndPoint);
                    ...
                    // Draw : sleeve
                    List<Entity> sleeveList = new List<Entity>();
                    List<Entity> sleeveTemp = new List<Entity>();
                    sleeveTemp = GetColumnSleeve(GetSumPoint(referencePoint, -(distanceRib + radiusOuter), basePlateHeight), distanceWorkPoint, scaleValue, columnBaseSupport);
                    Line sleeveLeftLn = (Line)sleeveTemp[3];
                    Line sleeveRightLn = (Line)sleeveTemp[1];

                    sleeveList.Add(sleeveLeftLn);
                    sleeveList.Add(sleeveRightLn);
                    sleeveList.Add((Line)sleeveTemp[2]);

                    Line guideSleeveLeftLn = new Line(GetSumPoint(sleeveLeftLn.EndPoint, 0, ribWidth), GetSumPoint(sleeveLeftLn.EndPoint, 0, -ribWidth));
                    Line guideSleeveRightLn = new Line(GetSumPoint(sleeveRightLn.EndPoint, 0, ribWidth), GetSumPoint(sleeveRightLn.EndPoint, 0, -ribWidth));

                    sleeveLeftLn.EndPoint = GetIntersectWidth(guideSleeveLeftLn, guideLeftSlopeLine);
                    sleeveRightLn.EndPoint = GetIntersectWidth(guideSleeveRightLn, guideLeftSlopeLine);
                    ...
                    // Draw : base TopPlate
                    // todo : Chanage Calculate length
                    // guideBaseTopLength : 45도 틀어서 옆에서 바라본 모습이라서 원래 길이를 45도 회전한 길이 사용
                    Line guideBaseTopLength = new Line(CopyPoint(referencePoint), GetSumPoint(referencePoint, baseTopPlateLength, 0));
                    guideBaseTopLength.Rotate(Utility.DegToRad(45), Vector3D.AxisZ, referencePoint);
                    double baseTopDiagonalLength = guideBaseTopLength.EndPoint.X - guideBaseTopLength.StartPoint.X;
                    /**/

                    List<Entity> baseTopTempList = new List<Entity>();
                    Point3D baseTopPlateSP = GetSumPoint(referencePoint, -baseTopDiagonalLength, baseBottomPlateThk);

                    // Sine@  Calculate
                    Line LeftEndLn = new Line(CopyPoint(referencePoint), GetSumPoint(referencePoint, 0, baseTopPlateThk));
                    LeftEndLn.Rotate(slopeRadian, Vector3D.AxisZ, referencePoint);
                    double LeftEndLnLength = LeftEndLn.EndPoint.Y - LeftEndLn.StartPoint.Y;

                    double LeftLnRadian = Utility.DegToRad(90) - selSlopeRadian;
                    double LeftLnLenght = baseTopPlateThk * Math.Sin(LeftLnRadian);

                    //Line baseTopPlateBtm = new Line(CopyPoint(baseTopPlateSP), GetSumPoint(baseTopPlateSP, baseTopDiagonalLength, 0));
                    //baseTopPlateBtm.Rotate(slopeRadian * dFlip, Vector3D.AxisZ, baseTopPlateBtm.EndPoint);
                    //Line baseTopPlateLeft = new Line(GetSumPoint(baseTopPlateBtm.StartPoint, LeftLnLenght,0), CopyPoint(baseTopPlateBtm.StartPoint));
                    //baseTopPlateLeft.Rotate(Utility.DegToRad(SlopeDeg) , Vector3D.AxisZ, baseTopPlateLeft.EndPoint);
                    //Line baseTopPlateTop = new Line(CopyPoint(baseTopPlateLeft.StartPoint), GetSumPoint(baseTopPlateSP, baseTopDiagonalLength, baseTopPlateThk));

                    Line baseTopPlateBtm = new Line(CopyPoint(baseTopPlateSP), GetSumPoint(baseTopPlateSP, baseTopDiagonalLength, 0));
                    Line baseTopPlateLeft = new Line(GetSumPoint(baseTopPlateBtm.StartPoint, 0, LeftEndLnLength), CopyPoint(baseTopPlateBtm.StartPoint));
                    baseTopPlateBtm.Rotate(slopeRadian * dFlip, Vector3D.AxisZ, baseTopPlateBtm.EndPoint);
                    baseTopPlateLeft.Rotate(slopeRadian * dFlip, Vector3D.AxisZ, GetSumPoint(baseTopPlateLeft.MidPoint, baseTopDiagonalLength, 0));
                    Line baseTopPlateTop = new Line(CopyPoint(baseTopPlateLeft.StartPoint), GetSumPoint(baseTopPlateSP, baseTopDiagonalLength, baseTopPlateThk));

                    baseTopTempList.Add(baseTopPlateBtm);
                    baseTopTempList.Add(baseTopPlateLeft);
                    baseTopTempList.Add(baseTopPlateTop);
                    ...
                    // Draw : base BottomPlate
                    // todo : Chanage Calculate length
                    // guideBaseBottomLength : 45도 틀어서 옆에서 바라본 모습이라서 원래 길이를 45도 회전한 길이 사용
                    Line guideBaseBottomLength = new Line(CopyPoint(referencePoint), GetSumPoint(referencePoint, baseBottomPlateLength, 0));
                    guideBaseBottomLength.Rotate(Utility.DegToRad(45), Vector3D.AxisZ, referencePoint);
                    double baseBottomDiagonalLength = guideBaseBottomLength.EndPoint.X - guideBaseBottomLength.StartPoint.X;
                    /**/

                    List<Entity> baseBottomTempList = new List<Entity>();
                    Point3D baseBottomPlateSP = GetSumPoint(referencePoint, -baseBottomDiagonalLength, 0);

                    double BottomLeftLnRadian = Utility.DegToRad(90) - selSlopeRadian;
                    double BottomLeftLnLenght = baseBottomPlateThk * Math.Sin(LeftLnRadian);

                    Line baseBottomPlateBtm = new Line(CopyPoint(baseBottomPlateSP), GetSumPoint(baseBottomPlateSP, baseBottomDiagonalLength, 0));
                    Line baseBottomPlateLeft = new Line(GetSumPoint(baseBottomPlateBtm.StartPoint, 0, BottomLeftLnLenght), CopyPoint(baseBottomPlateBtm.StartPoint));
                    baseBottomPlateBtm.Rotate(slopeRadian * dFlip, Vector3D.AxisZ, baseBottomPlateBtm.EndPoint);
                    baseBottomPlateLeft.Rotate(slopeRadian * dFlip, Vector3D.AxisZ, GetSumPoint(baseBottomPlateLeft.MidPoint, baseBottomDiagonalLength, 0));
                    Line baseBottomPlateTop = new Line(CopyPoint(baseBottomPlateLeft.StartPoint), GetSumPoint(baseBottomPlateSP, baseBottomDiagonalLength, baseBottomPlateThk));

                    baseBottomTempList.Add(baseBottomPlateBtm);
                    baseBottomTempList.Add(baseBottomPlateLeft);
                    baseBottomTempList.Add(baseBottomPlateTop);


                    if (i == 0)
                    {
                        outlinesList.AddRange(ribLeftList);
                        outlinesList.AddRange(sleeveList);
                        outlinesList.AddRange(baseTopTempList);
                        outlinesList.AddRange(baseBottomTempList);
                    }
                    else
                    {
                        outlinesList.AddRange(editingService.GetMirrorEntity(Plane.YZ, ribLeftList, referencePoint.X, referencePoint.Y, true));
                        outlinesList.AddRange(editingService.GetMirrorEntity(Plane.YZ, sleeveList, referencePoint.X, referencePoint.Y, true));
                        outlinesList.AddRange(editingService.GetMirrorEntity(Plane.YZ, baseTopTempList, referencePoint.X, referencePoint.Y, true));
                        outlinesList.AddRange(editingService.GetMirrorEntity(Plane.YZ, baseBottomTempList, referencePoint.X, referencePoint.Y, true));
                    }
            ...
            centerlinesList.AddRange(new Entity[] {
                //guideLeftSlopeLine,
                //guideRibLeftLn, guideRibRightLn
            });

            styleService.SetLayerListEntity(ref outlinesList, layerService.LayerOutLine);
            styleService.SetLayerListEntity(ref entityList, layerService.LayerOutLine);
            styleService.SetLayerListEntity(ref centerlinesList, layerService.LayerCenterLine);

            newList.AddRange(centerlinesList);
            newList.AddRange(entityList);
            newList.AddRange(outlinesList);
        ...
        private Point3D GetIntersectWidth(ICurve selCurve, ICurve selCurve2, int selNumber = 0)
        {
