...
namespace DrawSettingLib.SettingServices
{
    public class PaperService
    {
        ...
        #region CRT Centering Type
        public List<PaperModel> GetPaperModel_StructureCRTCenteringType(string selStructureType)
        {
            List<PaperModel> newList = new List<PaperModel>();


            if (selStructureType == "Rafter only (internal)")
            {
                newList.AddRange(GetPaperModel_StructureCRTInternalType());
            }
            else if (selStructureType == "Rafter only (external)")
            {
                newList.AddRange(GetPaperModel_StructureCRTExternalType());
            }
        ...
        private List<PaperModel> GetPaperModel_StructureCRTInternalType()
        {
            List<PaperModel> newList = new List<PaperModel>();

            newList.Add(GetPaperModel(PAPERMAIN_TYPE.DetailOfRoofStructure, 1, 1, 2));
            newList.Add(GetPaperModel(PAPERMAIN_TYPE.DetailOfRoofStructure, 2, 4, 12));

            return newList;
        }

        private List<PaperModel> GetPaperModel_StructureCRTExternalType()
        {
            List<PaperModel> newList = new List<PaperModel>();
